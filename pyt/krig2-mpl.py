    ...
    def play(self):
        ...
        while not self.player1.isEmpty() and not self.player2.isEmpty():
            ...
            while a.value == b.value:
                at_stake_p1.extend([self.player1.flip()
                                   for i in range(min(3, self.player1.getTotal()))])
                a = at_stake_p1[-1]

                at_stake_p2.extend([self.player2.flip()
                                   for i in range(min(3, self.player2.getTotal()))])
                b = at_stake_p2[-1]
...
# graph results
plt.plot(sorted(dist.keys()), [dist[i] for i in sorted(dist.keys())])
plt.show()
